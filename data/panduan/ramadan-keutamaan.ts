import { KeutamaanItem } from "@/types/panduan";

export const keutamaanItems: KeutamaanItem[] = [
  {
    id: "dalil-wajib-puasa",
    type: "hadith",
    arabic:
      "يَا أَيُّهَا الَّذِينَ آمَنُوا كُتِبَ عَلَيْكُمُ الصِّيَامُ كَمَا كُتِبَ عَلَى الَّذِينَ مِن قَبْلِكُمْ لَعَلَّكُمْ تَتَّقُونَ",
    translation:
      "Wahai orang-orang yang beriman, diwajibkan atas kalian berpuasa sebagaimana diwajibkan atas orang-orang sebelum kalian agar kalian bertakwa.",
    source: "QS. Al-Baqarah: 183",
  },
  {
    id: "keutamaan-puasa-ramadan",
    type: "hadith",
    translation:
      "Barangsiapa berpuasa Ramadan karena iman dan mengharap pahala, diampuni dosa-dosanya yang telah lalu.",
    source: "HR. Bukhari no. 38 & Muslim no. 760",
  },
  {
    id: "keutamaan-qiyamullail",
    type: "hadith",
    translation:
      "Barangsiapa menghidupkan malam Ramadan karena iman dan mengharap pahala, diampuni dosa-dosanya yang telah lalu.",
    source: "HR. Bukhari no. 37 & Muslim no. 759",
  },
  {
    id: "keutamaan-sedekah",
    type: "hadith",
    translation:
      "Barangsiapa memberi makan orang yang berbuka puasa, maka ia mendapatkan pahala seperti orang yang berpuasa tersebut tanpa mengurangi pahalanya sedikit pun.",
    source: "HR. Tirmidzi no. 807 — shahih",
  },
  {
    id: "lailatul-qadar",
    type: "hadith",
    arabic: "لَيْلَةُ الْقَدْرِ خَيْرٌ مِّنْ أَلْفِ شَهْرٍ",
    translation: "Malam Lailatul Qadar lebih baik dari seribu bulan.",
    source: "QS. Al-Qadr: 3",
  },
  {
    id: "puasa-sia-sia",
    type: "hadith",
    translation:
      "Betapa banyak orang berpuasa tapi tidak mendapatkan bagian dari puasanya itu kecuali rasa lapar dan dahaga. Betapa banyak orang mengerjakan shalat malam tapi tidak memperoleh dari shalatnya melainkan begadang dan rasa lelah.",
    source: "HR. Ahmad — shahih",
    commentary:
      "Hadits ini mengingatkan bahwa puasa bukan sekadar menahan makan dan minum, tapi menahan seluruh anggota tubuh dari dosa.",
  },
  {
    id: "salaf-sembunyikan-amal",
    type: "salaf-quote",
    translation:
      "\"Setan akan terus menggoda hingga hamba yang semula menyembunyikan amal akhirnya suka dipuji — dan catatan amal salehnya berubah menjadi riya'.\"",
    source: "Sufyan ats-Tsauri (atsar shahih)",
    commentary:
      "Para generasi salaf sangat antusias menyembunyikan amal ibadah mereka karena khawatir merusak keikhlasan. Sufyan memberikan peringatan keras tentang bahaya riya' yang tersembunyi.",
  },
  {
    id: "salaf-doa-6-bulan",
    type: "salaf-quote",
    subject: "Mu'alla bin Al-Fadhal",
    translation:
      "\"Mereka (para salaf) berdoa kepada Allah 6 bulan agar diberi umur hingga Ramadan, dan 6 bulan sesudahnya agar amal mereka diterima.\"",
    source: "Ibnu Rajab — Lathaif Ma'arif: 148",
  },
  {
    id: "ibnu-rajab-puasa",
    type: "salaf-quote",
    subject: "Ibnu Rajab Al-Hanbali",
    translation:
      "\"Ketahuilah bahwa puasa Ramadan memiliki dua tingkatan: (1) Meninggalkan makan, minum, dan syahwat. (2) Puasa seluruh anggota tubuh dari dosa.\"",
    source: "Ibnu Rajab — Lathaif Ma'arif",
  },
  {
    id: "ayub-sembunyikan-tahajjud",
    type: "salaf-example",
    subject: "Ayub as-Sikhtiyani",
    translation:
      "Mengerjakan shalat malam dan selalu merahasiakannya. Saat tiba Subuh, beliau mengangkat suara seakan baru terbangun.",
    source: "Riwayat dari para murid beliau",
    commentary:
      "Ini adalah contoh nyata bagaimana para salaf menjaga keikhlasan amal mereka.",
  },
  {
    id: "dawud-puasa-40-tahun",
    type: "salaf-example",
    subject: "Dawud bin Abi Hindun",
    translation:
      "Berpuasa selama 40 tahun tanpa diketahui keluarganya. Bekal makan siang dari istri selalu disedekahkan di jalan.",
    source: "Siyar A'lam An-Nubala'",
    commentary:
      "Dawud bin Abi Hindun adalah seorang tabi'in yang sangat menjaga keikhlasan amalnya.",
  },
  {
    id: "nasihat-salaf-ramadan",
    type: "salaf-quote",
    translation:
      "\"Para salaf tidak membuang waktu Ramadan walau sedetik. Malam untuk qiyam, siang untuk puasa, tilawah, dan dzikir.\"",
    source: "Ibnu Rajab — Lathaif Ma'arif",
    commentary:
      "Waspada: banyak pemuda masa kini yang menghabiskan malam-malam mulia Ramadan untuk hal sia-sia — ini adalah kerugian yang tidak bisa diganti.",
  },
];
