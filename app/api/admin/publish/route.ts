...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: latestContent, error: fetchLatestError } = await supabase
      .from("catatan_hsi")
      .select("*")
      .eq("id", id)
      .single();

    if (fetchLatestError || !latestContent) {
...
source: "${latestContent.source || "https://edu.hsi.id"}"
---
...
${latestContent.content}`;
    ...
    if (!transcriptionResponse.ok) {
      ...
      throw new Error(
        `GitHub API error: ${transcriptionResponse.status} ${error}`
      );
    }
